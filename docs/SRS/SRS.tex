...
\item[R\refstepcounter{reqnum}\thereqnum\label{R_In_measurement}:] \progname~shall accept the beacon measurements as a user input.

\item[R\refstepcounter{reqnum}\thereqnum\label{R_OutputInputs}:] \progname~shall compute $\mathbf{\hat{x}}$ based on the provided inputs (\rref{R_In_map}, \rref{R_In_position}, \rref{R_In_measurement}).

\item[R\refstepcounter{reqnum}\thereqnum\label{R_Calculate}:] \progname~shall calculate the estimated pose from the measurements provided for \iref{IM_MLE}.
...
\item[R\refstepcounter{reqnum}\thereqnum\label{R_Out_Visual}:] \progname~shall display a visual representation of the robot’s estimated position and orientation in a 2D environment.
