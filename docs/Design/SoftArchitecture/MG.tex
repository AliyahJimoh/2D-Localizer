...
\item[\refstepcounter{acnum} \actheacnum \label{acLocalize}:] The localization algorithm's accuracy.

\item[\refstepcounter{acnum} \actheacnum \label{acOutput}:] The format of the visualization method.

\item[\refstepcounter{acnum} \actheacnum \label{acSF}:] The integration of different sensors.

\item[\refstepcounter{acnum} \actheacnum \label{acFIM}:] The evaluation methods for accuracy.

\item[\refstepcounter{acnum} \actheacnum \label{acControl}:] The format for user interaction.

\item[\refstepcounter{acnum} \actheacnum \label{acCRLB}:] The technique for uncertainty estimation.
...
\item [\refstepcounter{mnum} \mthemnum \label{mSF}:] Sensor Fusion Module
...
\item [\refstepcounter{mnum} \mthemnum \label{mFIM}:] Fisher Information Matrix(FIM) Module
\item [\refstepcounter{mnum} \mthemnum \label{mCRLB}:] Cram\'{e}r-Rao Lower Bound(CRLB) Module
...
\multirow{3}{0.3\textwidth}{Software Decision Module} & FIM Module\\
& CRLB Module\\
& Plotting Module\\
\bottomrule
...
\item[Services:]Converts the localization and accuracy evaluation data into the data structure used by the
  input parameters module.
...
\subsubsection{Sensor Fusion Module (\mref{mSF})}

\begin{description}
\item[Secrets:]The algorithm that combines multiple sensor data from their sources (beacons and fiducial markers)
\item[Services:]Integrates sensor measurements to calculate joint position estimate
...
\subsubsection{FIM Module (\mref{mFIM})}
...
\subsubsection{CRLB Module (\mref{mCRLB})}
...
R4 & \mref{mInput}\\
R5 & \mref{mOutput}, \mref{mSF}, \mref{mLocalize}\\
R6 & \mref{mFIM}, \mref{mCRLB}\\
...
\acref{acSF} & \mref{mSF}\\
\acref{acFIM} & \mref{mFIM}\\
\acref{acControl} & \mref{mC}\\
\acref{acCRLB} & \mref{mCRLB}\\
