...
def localize(beacons, fm_map, fm_robot, range_m):
    ...
    graph.add(gtsam.PriorFactorPose2(robot_id, Pose2(8, 5, 0), prior_noise))
    ...
    for i, beacon_pos in enumerate(beacons):
        ...
        if range_m[i] < 15.0:
            graph.add(gtsam.RangeFactor2D(
                robot_id, beacon_symbol, range_m[i], range_noise))
    ...
    for i, beacon_pos in enumerate(beacons):
        if range_m[i] < 15.0:
            initial_estimates.insert(beacon_ids[i], Point2(
                beacon_pos[0], beacon_pos[1]))  # Fixed beacons as Point2
